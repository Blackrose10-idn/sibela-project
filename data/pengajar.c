...
void findAllPengajar(data *datas, int *nPage, SQLHDBC *dbConn, user *authUser)
{
    ...
    SQLAllocHandle(SQL_HANDLE_STMT, *dbConn, &stmt);
    SQLPrepare(stmt, (SQLCHAR *)"SELECT COUNT(*) AS row_count FROM pengajar WHERE id_pengajar LIKE ? OR nama LIKE ?", SQL_NTS);
...
void findAllPengajarSelect(Select *selectObject, SQLHDBC *dbConn)
{
    ...
    SQLAllocHandle(SQL_HANDLE_STMT, *dbConn, &stmt);
    SQLPrepare(stmt, (SQLCHAR *)"SELECT * FROM pengajar ORDER BY nama ASC OFFSET ? ROWS FETCH NEXT ? ROWS ONLY", SQL_NTS);
